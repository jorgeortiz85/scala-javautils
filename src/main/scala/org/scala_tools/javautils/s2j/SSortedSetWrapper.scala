...
trait SSortedSetWrapper[T] extends JSortedSet[T] with SSetWrapper[T] {
  type Wrapped <: SortedSet[T]
    ...
    override def first = underlying.firstKey

    override def last = underlying.lastKey

    override def comparator = new Comparator[T] {
        override def compare(a: T, b: T) = underlying.compare(a, b)
    }

    override def headSet(to: T) = new RichSSortedSet(underlying.until(to)).asJava

    override def subSet(from: T, to: T) = new RichSSortedSet(underlying.range(from, to)).asJava

    override def tailSet(from: T) = new RichSSortedSet(underlying.from(from)).asJava
}
